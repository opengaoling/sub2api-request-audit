...
// bound live client WebSocket sessions. It is independent from per-turn request
// slots because idle ingress connections should not occupy turn slots.
...
// OpenAIWSIngressLease keeps a Redis-backed ingress lease alive and cancels its
// context if Redis cannot confirm ownership for a full lease lifetime.
