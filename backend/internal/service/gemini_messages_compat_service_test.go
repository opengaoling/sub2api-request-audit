...
func TestGeminiForwardAsChatCompletions_FunctionNamedWebSearchStaysClientSide(t *testing.T) {
	...
	webSearchDeclaration, ok := functionDecls[0].(map[string]any)
	require.True(t, ok)
	readFileDeclaration, ok := functionDecls[1].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "web_search", webSearchDeclaration["name"])
	require.Equal(t, "read_file", readFileDeclaration["name"])
