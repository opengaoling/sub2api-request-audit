...
func TestForwardResponses_ItemTypeCompatibilityErrorFallsBackToChatCompletions(t *testing.T) {
	gin.SetMode(gin.TestMode)

	body := []byte(`{"model":"gpt-5.4","input":[{"type":"message","role":"user","content":[{"type":"input_text","text":"1+1="}]}],"stream":false}`)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	upstream := &httpUpstreamRecorder{responses: []*http.Response{
		{
			...
			Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid_llama_responses_item"}},
			Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"Cannot determine type of 'item'","type":"invalid_request_error"}}`)),
		},
		{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid_llama_chat_fallback"}},
			Body: io.NopCloser(strings.NewReader(
				`{"id":"chatcmpl_fallback","object":"chat.completion","model":"gpt-5.4","choices":[{"index":0,"message":{"role":"assistant","content":"2"},"finish_reason":"stop"}],"usage":{"prompt_tokens":4,"completion_tokens":1,"total_tokens":5}}`,
			)),
		},
	}}
	svc := &OpenAIGatewayService{
	...
	require.Equal(t, "1+1=", gjson.GetBytes(upstream.bodies[1], "messages.0.content").String())
	...
	require.Equal(t, "2", gjson.Get(rec.Body.String(), "output.0.content.0.text").String())
}
