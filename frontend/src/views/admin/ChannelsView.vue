...
async function handleSubmit() {
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    if (section.group_ids.length === 0) {
      ...
      appStore.showError(t('admin.channels.noGroupsSelected', { platform: platformLabel }, `${platformLabel} has no selected group. Select at least one group or disable this platform.`))
    ...
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        appStore.showError(t('admin.channels.emptyModelsInPricing', { platform: platformLabel }, `${platformLabel} has a pricing entry without models. Add models or remove the entry.`))
        activeTab.value = section.platform
        return
      }
    }
    ...
    if (pricingConflict) {
      appStore.showError(
        t('admin.channels.modelConflict',
          ...
          `Model patterns '${pricingConflict[0]}' and '${pricingConflict[1]}' conflict: overlapping match range.`)
      )
      activeTab.value = section.platform
      return
    }
      ...
      if (mappingConflict) {
        appStore.showError(
          t('admin.channels.mappingConflict',
            ...
            `Mapping source patterns '${mappingConflict[0]}' and '${mappingConflict[1]}' conflict: overlapping match range.`)
        )
        activeTab.value = section.platform
        return
      }
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    for (const entry of section.model_pricing) {
      ...
      if ((entry.billing_mode === 'per_request' || entry.billing_mode === 'image') &&
        ...
        appStore.showError(t('admin.channels.form.perRequestPriceRequired', 'Per-request price or billing tiers required for per-request/image billing mode'))
        return
      }
    }
  }
